...
\title{Causal Discovery under Latent Confounding}
...
% \end{IEEEkeywords}
    ...
    Causal discovery, which involves discovering cause-effect relationships \cite{2009pearl}, plays a significant role in scientific enquiry in the domains of economics \cite{2025garg}, medicine \cite{2022sanchez}, biology \cite{2016lagani}, and climate science \cite{2012imme}% \cite{2019runge}
    , among others. Randomised Control Trials (RCT) are considered to be the standard technique for determining causal relationships \cite{2004stolberg} \cite{2015imbens}. However, they might turn out to be prohibitively expensive, impractical or immoral to carry out in the real world. This is where algorithms for causal discovery, solely from observational data are highly useful. 
    ...
    Existing RL approaches handle acyclicity through incorporating the regularization term into the reward or through flow-based auto-regressive methods that sequentially add edges, checking for acyclicity at every step \cite{2022deleu}. The former fails to guarantee acyclicity, while the latter suffers from computational inefficiency due to sequential graph generation.
    ...
    \section{Methodology}
    \subsection{Overview}
    The proposed algorithm, given an input observational dataset, works by repeatedly executing the below steps for a specified number of iterations: First, use a continuous optimization algorithm (PPO and CMA-ES in our case) to search through a high dimensional Euclidean space. Next, use the surjective \textsc{Vec2ADMG} mappings to convert the sampled points into bow-free or ancestral ADMG graphs. Finally, use the RICF algorithm to compute the BIC score of the graph on the provided dataset. This is the score that the optimization algorithm attempts to minimize (see figure \ref{fmwk_fcht} in the appendix for a diagrammatic representation).

    The modular nature of the algorithm allows one to swap-in different components, like optimization algorithms or score metrics, allowing the algorithm to be easily adapted and extended to different use-cases as long as the conditions for identifiability and consistency are satisfied, and a suitably fast mapping to the intended class of graphs can be found.
    ...
    \begin{definition}[Vec2ADMG]
        \label{vec2admg}
        For all \(d \in \mathbb{N}^{+} \textnormal{ and } z \in \mathbb{R}^{d^2}\), let \(p \in \mathbb{R}^d\) be the vector formed from the first \(d\) elements of \(z\), and let \(E_{\rightarrow}, E_{\leftrightarrow} \in \mathbb{R}^{d \times d}\) be strictly lower triangular matrices formed from the next \(\frac{d(d-1)}{2}\) and final \(\frac{d(d-1)}{2}\) elements of \(z\), respectively. Then, the functions \(\Phi_{d}^{BF}\) and \(\Phi_{d}^{AN}\) constitute the Vec2ADMG mapping as follows:
        \begin{align*}
    ...
    The \(H(E + E^{\top})\) terms in definition \ref{vec2admg} control edge existence. \(E\) is used to refer to both the edge potential matrices since they behave similarly. \(H(E + E^{\top})_{ij} = 1\) is a necessary but not sufficient condition for the edge \(i \rightarrow j\) to exist. The Heaviside step function applied on top of the gradient operator means that a directed edge \(i \rightarrow j\) is allowed to exist only if \(p_i < p_j\).%: \(H(\textnormal{grad}(p))_{ij} = 1 \Leftrightarrow p_i < p_j\). 
    The causal order of the nodes, thus, becomes the order of the indices of the elements of \(p\) when they are considered in ascending order. \(H(E + E^{\top})\) controls whether there is an edge between two nodes, while \(H(\textnormal{grad}(p))\) filters out some of the edges to enfore a causal ordering. \(D\), thus represents the DAG component of the ADMGs. 
    ...
    We provide the proof in Appendix \ref{lemma_1_proof}. The scale invariance of \(\Phi_d\) is leveraged in the construction of the objective function for the CMA-ES algorithm. The optimizer can increase the distance of the candidate solutions from the decision boundaries to increase stability without altering the logical structure of the graph.
    ...
    We provide the proof in Appendix \ref{lemma_2_proof}. The above lemma means that the entire class of ADMGs can be generated by the vectors found within an arbitrarily small neighbourhood of the origin. This ensures that all ADMGs are theoretically reachable by a continuous optimization algorithm, and there are no graphs that might require the algorithm to traverse to infinity to recover.
    ...
    Policy Gradient approaches only guarantee convergence to local optima, but we can incentivise the model to exit local minima by adding a entropy regularization term to the PPO loss. Since we actually only care about the best action (the \(z\) value with the least BIC score) found so far, rather than the final policy to which the model converges, we can additionally do an annealed cycling of the entropy coefficient so that when its value is high, the magnitude of the variance term increases and the model explores more, while when its value is low, the variance decreases and the model can converge to a local optima. The annealing is to ensure eventual model convergence.

    \subsection{Optimization with CMA-ES}
    The objective function to be minimized by the CMA-ES algorithm is a margin-maximised BIC score:
    \begin{align}
    ...
    The CMA-ES algorithm uses a Gaussian to sample points from the search space. Similar to the restriction for PPO, we restrict the Gaussian to be diagonal in order to ensure scalability. The CMA-ES algorithm terminates when, among other reasons, the difference in the best objective function values over the last few generations falls below a set threshold, or the elements of the standard deviation matrix of the search distribution fall below a set threshold. To allow the CMA-ES algorithm to escape these local minima, just like the entropy cycling of the PPO algorithm, we allow the CMA-ES algorithm to do restarts, each time with slightly larger starting standard deviation, until the limit of the number of function evaluations is reached.

    \section{Experiments}
    \subsection{Algorithms and Implementations}
    The version of our algorithm\footnote{https://github.com/TheDansmod/qmul-msc-ai-code-master-thesis} that uses PPO is labelled \textit{Relcadilac} (\underline{Re}inforcement \underline{L}earning for \underline{Ca}usal \underline{Di}scovery under \underline{La}tent \underline{C}onfounding) and the version using CMA-ES is called \textit{CMA-ES}. Comparison is made with the \textit{DCD} algorithm \cite{2021bhattacharya} and the \textit{GFCI} algorithm \cite{2016ogarrio}. The authors' original implementation\footnote{https://gitlab.com/rbhatta8/dcd} was used for the DCD algorithm while for the GFCI algorithm we use the \lstinline[basicstyle=\ttfamily, breaklines=True]{py-tetrad}\footnote{https://github.com/cmu-phil/py-tetrad} python library \cite{2023ramsey} which is a wrapper around the Tetrad suite of programs \cite{2018ramseya}. The hyperparameters for Relcadilac and CMA-ES are provided in Appendix \ref{algo_hyp}, while default configurations were used for DCD and GFCI. The Stable-Baselines3 \cite{2021raffin} python library was used for the PPO algorithm while the \lstinline[basicstyle=\ttfamily, breaklines=True]{cma}\footnote{https://github.com/CMA-ES/pycma} \cite{2019hansen} python library was leveraged for their implementation of the CMA-ES algorithm.
    ...
    The identifiablity of ancestral ADMGs means that we can directly compare ADMG predictions from the algorithms with the ground truth ADMG graph. Bow-free ADMGs, being only almost-everywhere identifiable, means that there might be more than one ADMG which is observationally equivalent to the ground truth ADMG. Owing to this, following the same logic as \cite{2021bhattacharya}, our algorithm first converts the proposed ADMG to a MAG, then converts the MAG to a PAG using the java-based Tetrad library \cite{1998scheines}. Our algorithm reports both the proposed bow-free or ancestral ADMG and the derived PAG. While the DCD algorithm also provides both the ADMG and the corresponding PAG, the GFCI algorithm only outputs a PAG. Thus, the GFCI algorithm is only included when comparing metrics derived from the PAG.

    The primary metrics we compare are the Structural Hamming Distance against the ground truth ADMG graph, and the \(F_1\) score for edge adjacencies computed on the PAG skeleton. The SHD score computes the number of addition and deletion operations (reversels therefore count as two operations) that need to be performed on the edges of the proposed graph to convert it to the ground truth graph. Additions or removal of bidirected edges only count as one operation. Graphs for other metrics like fractional BIC excess, which computes how much larger the predicted BIC score is than the true BIC score as a fraction of the true BIC score, and runtime graphs are presented in appendix \ref{graphs_appendix}. The graphs comparing the performance of the algorithms with variation in fraction of directed edges and in the average degree are also delegated to appendix \ref{graphs_appendix} due to space constraints.

    To move beyond synthetic data, we evaluate the proposed algorithms on the real world protein signalling networks dataset called the Sachs dataset \cite{2005sachs}. The ground truth graph of the Sachs dataset is not consistent despite its widespread use (like in \cite{2018ramseyb} and \cite{2016mooji}), so we use the graph from Figure 2 of the original paper (see reproduction in figure \ref{sachs_gt} in the appendix). The total number of edges in the Sachs dataset is 16 where one of the edges is bidirected, and it has 11 nodes. The observational component of the Sachs dataset is only 853 samples. This is likely the reason why the BIC scores for the proposed graphs are all lower than the BIC scores for the true graph. See table \ref{sachs_table} in the appendix for the results.
    ...
    The Relcadilac algorithm (where we use PPO for optimization) often performs poorly on most experiments when compared to the performance of the CMA-ES and DCD algorithms. It mostly outperforms the GFCI algorithm on comparable metrics. The likely cause for this is the sample inefficiency of the PPO algorithm \cite{2018haarnoja} \cite{2018henderson}. The authors in \cite{2025duong}, who use PPO for DAG search, run their algorithm for \(1.28 \times 10^6\) function evaluations. Since they only need to compute the BIC score for DAGs, they are able to use the Sum of Square Residuals formulation for their calculation.
    % \[S_{BIC}(X, \mathcal{G}) = -\left(n\sum_{i=1}^{d}\ln \frac{\textnormal{SSR}_i}{n} + |\mathcal{G}|\ln n\right)\]
    The likelihood term in this score can be decomposed on a per-node basis, and thus, the contribution of each node and potential parent set is fixed, given the dataset. This allows partial likelihood scores to be saved and re-used throughout the learning process, considerably speeding up the computation, especially for sparse or small graphs.
    ...
    \begin{figure}[!t]
        ...
        \caption{The ground truth graph of the Sachs Dataset \cite{2005sachs} derived from Fig 2. in the original paper. \label{sachs_gt}}
    \end{figure}
    \label{algo_hyp}
    ...
    \begin{figure}[!t]
        \centering
        \includegraphics[width=0.49\textwidth]{cadilac_diagram.pdf}
        \caption{\textbf{A flowchart for the Cadilac Framework} The diagram depicts that the inputs to the proposed framework, called Cadilac, are the data and the graph class \(\mathcal{G}\) which could be either bow-free or ancestral, and that the outputs of the algorithm are the candidate ADMG graph and the PAG derived from the candidate. In the algorithm, we use continuous optimization algorithms like CMA-ES and PPO to search through a high dimensional Euclidean search space. These sampled points are then converted to the space of either ancestral or bow-free ADMGs using the novel surjective \textsc{Vec2ADMG} mappings. Once we have the graph, the BIC score of the graph is computed using the RICF algorithm and the input observational data. The CMA-ES and PPO algorithms update their parameters based on the BIC scores of the sampled graphs. This loop continues for a specified number of iterations. For CMA-ES the default is 40,000 and for PPO the default is 80,000. \label{fmwk_fcht}}
    \end{figure}
    ...
    \section{RICF Implementation Comparison}
    \label{ricf_comp}
    BIC scores were computed using my modification of \lstinline[basicstyle=\ttfamily, breaklines=True]{ananke-causal} python library's \cite{2023lee} implementation of the RICF algorithm and the unmodified implementation. The runtime of the algorithm's (in seconds) were compared in table \ref{ricf_table} with varying number of nodes \(d\) in the graph. All metrics were calculated while comparing the algorithms on the same data. Average degree of the nodes was sampled uniformly from all valid degrees (between 2 and \(d-1\)). The fraction of directed edges in graphs was sampled uniformly from 0.2 to 0.7. The graph class was randomly chosen between bow-free and ancestral. The number of samples in the graphs was restricted to be 2000 for all runs. As is apparent from the data in the table, my algorithm consistently provides at least a 2x speedup over the library implementation.
    \begin{table}[!b]
        \caption{RICF Runtime comparison in seconds \label{ricf_table}}
        \begin{tabular}{c | l l l}
            \hline
            \textbf{Number of Nodes} & \textbf{My Implementation} & \textbf{Ananke} & \textbf{Ratio} \\ \hline
            5 & \(0.002 \pm 0.001\) & \(0.011 \pm 0.003\) & 5.50 \\
            10 & \(0.014 \pm 0.020\) & \(0.040 \pm 0.056\) & 2.86 \\
            15 & \(0.038 \pm 0.049\) & \(0.104 \pm 0.230\) & 2.74 \\
            20 & \(0.108 \pm 0.148\) & \(0.371 \pm 1.130\) & 3.44 \\
            25 & \(0.270 \pm 0.342\) & \(14.706 \pm 95.367\) & 54.47 \\
            30 & \(0.399 \pm 0.484\) & \(3.049 \pm 9.300\) & 7.64 \\
            \hline
        \end{tabular}
    \end{table}
\end{document}
