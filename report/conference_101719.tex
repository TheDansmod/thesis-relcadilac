...
% \usepackage[style=authoryear, giveninits=false, minbibnames=4, maxbibnames=5]{biblatex}
% \usepackage[style=authoryear, giveninits=false, minbibnames=4, maxbibnames=5, maxcitenames=1, mincitenames=1]{biblatex}
% \addbibresource{/mnt/windows/Users/lordh/Documents/LibraryOfBabel/Education/queen_mary_master_ai/courses/msc_project/notes/references.bib}
% to use square brackets rather than parenthesis when using cite
% \renewcommand*{\mkbibparens}[1]{[#1]}
% to increase spacing between references in the bibliography
% \setlength{\bibitemsep}{0.5\baselineskip}
% to have the order of all names be initials/first name last-name
...
% \end{IEEEkeywords}
    ...
    \begin{enumerate}
        ...
        \item Use the surjective vector-to-admg mappings to convert the sampled points into bow-free or ancestral ADMG graphs.
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.48\textwidth]{/mnt/windows/Users/lordh/Documents/LibraryOfBabel/Projects/thesis/diagrams/to_include_in_paper/best_bic_so_far.pdf}
        \caption{The graph plots the best BIC score found at each time step for the Relcadilac algorithm. This run was made for a 20 node ancestral graph, 4 average degree (meaning 40 edges on average), 2000 sample size, running for 2 hours and 43 minutes. The final SHD score obtained for the 20-node graph was 35, which is in contrast to the 161 average 20-node SHD seen in figure \ref{fig_shd_pag_skeleton_f1_nodes_samples}. The graph is intended to show the large sample requirement (number of environment interactions) of the PPO algorithm before it provides results comparable to CMA-ES and DCD, especially on larger node graphs.}
