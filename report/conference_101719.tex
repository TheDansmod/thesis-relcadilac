...
% \end{IEEEkeywords}
    ...
    A number of causal discovery methods assume \textit{causal sufficiency}, where no common cause of the set of variables of interest is extraneous to the set \cite{2022zanga}. This assumption is often violated in practice \cite{2006kesteloot} and algorithms relying on this assumption might predict spurious relationships between variables. Acyclic Directed Mixed Graphs (ADMGs) \textcolor{red}{[cite paper that defines ADMGs]}, which utilize directed edges to depict cause-effect relationships between variables and bidirected edges to indicate the presence of confounders, are often used to model data containing unobserved confounders \textcolor{red}{[cite some papers that use ADMGs]}.

    \textcolor{Green}{Score-based methods:} Score-based causal discovery methods work by assigning a score \(S(G)\) to each candidate graph \(G\) and find the best graph \(G^*\) in some class of graphs by minimising the score while searching over the space of all possible graphs in that class. Common scores used for this purpose include the Akaike Information Criterion (AIC) \cite{1974akaike}, the Bayesian Information Criterion (BIC) \cite{1978schwarz} (which we use), and the Bayesian Dirichlet equivalent uniform (BDeu) \cite{1994geiger}. 
    \begin{equation}
