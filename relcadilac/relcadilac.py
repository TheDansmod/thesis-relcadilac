...
def relcadilac(
    ...
    try:
        ...
        # below line was changed - dict was put last and best bic second last - was reverse initially so if you are running experiments file in relcadilac folder that will not work. the latest experiments file is in cma_es folder
        return pred_D.astype(int), pred_B, pag_matrix, best_bic, {'average_rewards': tracking.average_rewards}
    finally:
        vec_env.close()
        if 'model' in locals():
            del model
