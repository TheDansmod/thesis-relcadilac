...
def acyclify(A):
    ...
    A_abs = np.abs(A)
    m = A.shape[1]
    # danish mod: the _h function needs a second argument but did not have one, I have added it
    while _h(A_abs, m) > 0:
        ...
        return A
    A[A_abs == 0] = 0
    # danish mod: the below line was returning _A, but that does not exist, so I have changed it to A, which is what I think it should be returning
    return A
