...
class MatMulLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        # danish mod: updated to latest keras 3.0
        self.kernel = self.add_weight(name="A", shape=[self.input_dim, self.input_dim])
...
class CustomMatMulLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        # danish mod: updated to latest keras 3.0
        self.kernel = self.add_weight(name="B", shape=[self.latent_dim, self.input_dim])
    ...
    for i in range(k_max_iter):
      while c_A < 1e20:
        for _ in range(epochs):
          ...
          if loss < best_loss:
            # danish mod: added .numpy().copy() to below 2 lines
            best_A = model.A.numpy().copy()
            best_B = model.B.numpy().copy()
            best_ca = c_A
            best_la = lam_A
          last_A = model.A
          last_B = model.B
